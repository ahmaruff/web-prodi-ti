<x-app-layout>
    ...
    <div class="my-6 p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div>
            <div class="h-48 bg-purple-400">
                <img src="{{url('/storage/uploads/fasilitas/'.$fasilitas->image)}}" alt="gambar fasilitas" class="object-contain w-full h-48">
            </div>
            <form action="{{route('admin.fasilitas.update', $fasilitas->id)}}" method="post" enctype="multipart/form-data">
                ...
                <label for="nama" class="block my-4">
                    ...
                    <input type="text" name="nama" id="" value="{{$fasilitas->nama}}" placeholder="Nama Fasilitas" required
                ...
                <label for="title" class="block mt-3">
                    ...
                    <textarea name="deskripsi" id="" class="w-full mt-1 min-h-[7rem] bg-gray-100 border-transparent rounded-md dark: focus:border-purple-500 focus:bg-white focus:ring-0 dark:bg-dark-eval-2">{{$fasilitas->deskripsi}}</textarea>
                </label>
