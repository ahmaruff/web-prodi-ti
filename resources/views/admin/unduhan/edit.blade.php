<x-app-layout>
    ...
    <div class="p-6 my-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div x-data="{ is_link : '' }">
            <form action="{{route('admin.unduhan.update', $unduhan->id)}}" method="post" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
                <label for="judul" class="block my-4">
                    <span class="">Judul Dokumen</span>
                    <input type="text" name="judul" id="" value="{{$unduhan->judul}}" placeholder="Dokumen" required
                ...
                <label for="title" class="block mt-3">
                    ...
                    <textarea name="deskripsi" id="" class="w-full mt-1 min-h-[7rem] bg-gray-100 border-transparent rounded-md dark: focus:border-purple-500 focus:bg-white focus:ring-0 dark:bg-dark-eval-2">{{$unduhan->deskripsi}}</textarea>
                </label>
                <label for="jenis_dokumen" class="block mt-3">
                    <span class="block mb-1">Jenis Dokumen</span>
                    <div class="flex items-center mb-2">
                        <input type="radio" name="jenis_dokumen" value="file" x-on:click="is_link=false"
                ...
                <label for="path" class="block my-4" x-show="is_link">
                    ...
                    <input type="url" name="link" id=""  placeholder="URL Dokumen" @if ($unduhan->is_link) value="{{$unduhan->path}}" @endif
                    class="block w-full mt-1 bg-gray-100 border-transparent rounded-md dark: focus:border-purple-500 focus:bg-white focus:ring-0 dark:bg-dark-eval-2"
                    >
                </label>
                <label for="path" class="block my-4" x-show="!is_link">
