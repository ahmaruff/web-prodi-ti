<x-app-layout>
    ...
    <div class="my-6 p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div>
            <form action="{{route('admin.himti.proker.update', $program_kerja->id)}}" method="post">
                @method('PUT')
                @csrf
                <label for="judul" class="block my-4">
                    <span class="">Judul</span>
                    <input type="text" name="judul" id="" value="{{$program_kerja->judul}}" placeholder="Pelatihan IoT" required
                ...
                <label for="deskripsi" class="block mt-3">
                    ...
                    <textarea name="deskripsi" id="" class="w-full mt-1 min-h-[7rem] bg-gray-100 border-transparent rounded-md dark: focus:border-purple-500 focus:bg-white focus:ring-0 dark:bg-dark-eval-2" required>{{$program_kerja->deskripsi}}</textarea>
