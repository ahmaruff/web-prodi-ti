<x-app-layout>
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div class="">
            <form action="{{route('admin.event-mahasiswa.store')}}" method="post" enctype=multipart/form-data
            x-data="{content : {html : '', json : ''}}"
            >
                {{ csrf_field() }}
                <label for="title" class="block">
                    <span class="">Judul</span>
                    <input type="text" name="judul" id="" value="" placeholder="Judul Event" required
